...
namespace PieChart
{
    public class Helper
    {
        ...
        public static void FindDownInTree(List<FrameworkElement> lst, DependencyObject parent, DependencyObject ignore,
            IFinderMatchVisualHelper helper)
        {
            for (int i = 0; i < VisualTreeHelper.GetChildrenCount(parent); i++)
            {
                ...
                if (helper.DoesMatch(visual)) lst.Add(visual as FrameworkElement);

                if (lst.Count > 0 && helper.StopAfterFirst)
                    break;

                if (visual != ignore) FindDownInTree(lst, visual, ignore, helper);
            }
        }
        ...
        /// <param name="ignore"></param>
        /// <param name="helper"></param>
        public static void FindUpInTree(List<FrameworkElement> lst, Visual parent, Visual ignore,
            IFinderMatchVisualHelper helper)
        {
            // First thing to do is find Down in the existing node...
            //FindDownInTree(lst, parent, ignore, helper);

            if (helper.DoesMatch(parent)) lst.Add(parent as FrameworkElement);
            ...
            } else
            {
                ...
                if (parent is FrameworkElement feCast)
                {
                    ...
                    if (feNewParent != null && feNewParent != feCast) FindUpInTree(lst, feNewParent, feCast, helper);
                }
            }
        }
        ...
        public class FinderMatchType : IFinderMatchVisualHelper
        {
            ...
            public FinderMatchType(Type type)
            {
                _type = type;
            }

            public FinderMatchType(Type type, bool stopAfterFirst)
            {
                _type = type;
